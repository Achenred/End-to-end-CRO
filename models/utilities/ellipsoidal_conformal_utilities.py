...
def local_ellipse_validity_efficiency(
    ...
    for i in range(local_neighbors_test.shape[0]):
        ...
        cov_list.append(local_cov_test_regularized)
        local_inv_cov_test = np.linalg.inv(local_cov_test_regularized)

        local_error_test = (y_true_test[i, :] - y_pred_test[i, :]).detach().numpy()
    ...
    # a = local_non_conf_multi_test_all[:10]
    # b = local_alpha_s*0.5
    # c = [1 if i <= b  else 0 for i in  local_non_conf_multi_test_all[:10]]
    
    
    binary_flag = [1 if i <= local_alpha_s*0.5  else 0 for i in  local_non_conf_multi_test_all]
    ...
    return normalized_ellipse_validity, normalized_ellipse_efficiency, binary_flag, cov_list
